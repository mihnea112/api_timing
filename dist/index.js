...
app.post("/api/time", async (req, res) => {
    ...
    try {
        const dbResult = await db.execute("INSERT INTO time_logs (time_ms) VALUES (?)", [time_ms]);
...
app.get("/api/times", async (req, res) => {
    try {
        const [rows] = await db.execute("SELECT * FROM time_logs ORDER BY created_at DESC");
        res.json(rows);
    }
    catch (err) {
        console.error(err);
